...
# Flaskのルート定義の前に、トップレベルで初期化関数を呼び出す
setup_ai_model()
print("setup関数に入りました")
# 1. まず、Flaskアプリ(WSGI)をASGI互換のアプリに変換する
asgi_app = WsgiToAsgi(app)
# 2. 次に、ASGI互換になったアプリをMangumでラップする
...
def generate_quiz():
    print(f"★★★ 404 DEBUG ★★★ Request path received: {request.path}")
    print(f"★DEBUG★ Received request path: {request.path}")
    print(f"★DEBUG★ Full request headers: {request.headers}")
...
@app.route('/default/MyQuizBackendFunction-Container/verify-deployment-v3')
def verify_deployment():
    # このメッセージがログに出れば、この関数が実行されたことが確定する
    print("★★★ Deployment Verification v3 SUCCESS ★★★")

    # このJSONがブラウザに表示されれば、API全体が正常に機能している
    return {
        "status": "ok",
        "message": "Deployment verification route version 3 is running!",
        "timestamp": "2025-06-24-1212"  # 変更が反映されたか確認するためのタイムスタンプ
    }

@app.route('/test')
def simple_test_route():
    print("★★★ /test route was successfully executed! ★★★")
    return {"message": "Success from /test route!"}
